...
test('operator evaluates corridors and sees a recommended segment', async ({ page }) => {
  ...
  const submitButton = page.getByRole('button', { name: 'Run offer evaluation' })
  const maximumDistance = page.getByTestId('maximum-distance')
  await expect(submitButton).toBeEnabled()
  await maximumDistance.fill('900')
  await expect(maximumDistance).toHaveValue('900')
